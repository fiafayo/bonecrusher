...
/* 
   ...
   UPDATE  		: 
   KEGUNAAN     : ENTRY jumlah DIRECT COST per jurusan
...
if ($act==1)   
{ 
	if (($frm_thn=='') or ($frm_jurusan=='') or ($frm_jum_dr_cost=='')) 
		{
			$error = 1;
			$pesan=$pesan."<br>Maaf, Anda harus mengisi Tahun, Jurusan, Jumlah Direct Cost";
		}
	if ($error==1) 
		{
	...
	if ($error !=1) // Jika semua isian form valid 
		{
				...
				{
					echo "<br>INSERT";
					$result = mysql_query("INSERT INTO direct_cost (`id`, `tahun` , `jurusan`, `dr_cost`) VALUES 
					( NULL, ".$frm_thn.", $frm_jurusan, $frm_jum_dr_cost) " );
					if ($result) 
						{
			    ...
			    else
				{
					...
					echo "<br>frm_id_dr_cost=".$frm_id_dr_cost;
					...
					$result = mysql_query("UPDATE direct_cost SET `tahun` = $frm_thn,
															  `jurusan` = $frm_jurusan, 
														      `dr_cost` = $frm_jum_dr_cost
													    WHERE  id = $frm_id_dr_cost");
...
$result = mysql_query("DELETE FROM direct_cost WHERE id = ".$id_dr_cost);
...
if (($act!=0)and($error!=1)) 
{
	//$frm_id_dr_cost="";
	$frm_jurusan=0;
	$frm_id_dr_cost=$row["id"];
	$frm_thn=$row["tahun"];
	$frm_jurusan=$row["jurusan"];
	$frm_jum_dr_cost=$row["dr_cost"];
}
else
{
//echo "HERE";
...
<body class="body">
<form name="form_sc_dr_cost" id="form_sc_dr_cost" action="sc_dr_cost.php" method="post">
...
<tr>
      <td width="16%"><? //echo "<br>frm_id_dr_cost=".$frm_id_dr_cost;?></td>
      <td width="2%"><div align="center"></div></td>
      <td width="82%">
		  <input type="hidden" name="frm_id_dr_cost" id="frm_id_dr_cost" value="<?php echo $frm_id_dr_cost;?>">
    ...
    <tr>
      ...
      <td><div align="center"><strong>:</strong></div></td>
      <td nowrap>
	  <select name="frm_jurusan" id="frm_jurusan" class="tekboxku" onChange="document.form_sc_dr_cost.submit();">
    ...
    <tr>
      <td nowrap>Jumlah Direct Cost </td>
      <td><div align="center"><strong>:</strong></div></td>
      <td><font color="#FF0000">
        <input name="frm_jum_dr_cost" id="frm_jum_dr_cost" onKeyPress="return cek_digit(event)" value="<? echo $frm_jum_dr_cost;?>" type="text" class="tekboxku" size="30" maxlength="50" >
*      </font></td>
    </tr>
    <tr>
      <td nowrap>&nbsp;</td>
    ...
    <tr>
      ...
      <td>
		...
		<?php if ($frm_id_dr_cost) { ?>
		<input name="Submit3" type="button" class="tombol"   disabled  onClick="if(confirm('Hapus ?')){this.form.action+='?act=2&id_dr_cost=<?php echo $frm_id_dr_cost;?>';this.form.submit()};" value="Hapus">
		<?php } ?>
	  </td>
    </tr>
    <tr> 
