...
$act = $_GET['act'];
/*
echo "<br>frm_no_induk_wasit=".$frm_no_induk_wasit;
...
// NRP dan NAMA harus diisi
	...
	if ($error !=1) // Jika semua isian form valid 
		{
$result_cek = mysql_query("SELECT nama, no_induk_wasit
							 FROM wasit 
							WHERE no_induk_wasit='".$_POST['frm_no_induk_wasit']."'");
			...
			if ($jumlah_rows!=1) 
				{
					...
					$result = mysql_query("INSERT INTO wasit (no_induk_wasit, nama, kelamin, tempat_lahir, tanggal_lahir, alamat, kota_wasit, propinsi_wasit, email)VALUES('".$frm_no_induk_wasit."', '".$frm_nama."', '".$frm_jenis_kelamin."', '".$frm_tempat_lahir."', '".datetomysql($frm_tgl_lahir)."', '".$frm_alamat."', '".$frm_kota."', '".$frm_propinsi."', '".$frm_email."')" );
  				    if ($result) 
						{
			...
			else
				{
					           $result = mysql_query(" UPDATE wasit SET `nama` = '$frm_nama',
																		...
																		`email`= '$frm_email'
													              WHERE `no_induk_wasit` = '$frm_no_induk_wasit'");
					if ($result) 
						{
...
$result = mysql_query("DELETE FROM wasit WHERE no_induk_wasit = '".$frm_no_induk_wasit."'");
...
if ($frm_nama!='')  {
  $result = mysql_query("SELECT `no_induk_wasit`,
								`nama`,
								`kelamin`,
								`tempat_lahir`,
...
<table width="90%" border="1">
  ...
  <tr>
    <td>
    <form id="form_wasit" name="form_wasit" action="index.php?menu=wasit" method="post" onsubmit="">
    <table width="95%" border="0" align="center">
      ...
      <tr>
        <td>No. Induk Wasit</td>
        <td>:</td>
        <td>
          <input type="text" name="frm_no_induk_wasit" id="frm_no_induk_wasit" value="<?=$frm_no_induk_wasit;?>" />
          </td>
      </tr>
      <tr>
        <td>Jenis kelamin</td>
