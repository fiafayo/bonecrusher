...
echo "<br>frm_nama_club=".$frm_nama_club;
...
// NRP dan NAMA harus diisi
	if (($frm_no_ID_club=='') or ($frm_nama_club=='')) 
		{
			$error = 1;
	...
	if ($error !=1) // Jika semua isian form valid 
		{
			$result_cek = mysql_query("SELECT *
						 				 FROM club 
										WHERE (nama='".$_POST['frm_nama_club']."' AND id_club=".$_POST['frm_no_ID_club'].")");
			if ($result_cek) 
				{
					$frm_exist=1;
		...
		// data id tidak ada, berarti record baru
			if (($frm_exist!=1) AND ($frm_no_ID_club==""))
				{
					echo"inset";
					$result = mysql_query("INSERT INTO club (id_club, nama, alamat, kota_club, propinsi_club)VALUES(".$frm_no_ID_club.", '".$frm_nama_club."','".$frm_alamat."',".$frm_kota.", ".$frm_propinsi."')" );
  				    if ($result) 
						{
							$pesan = $pesan."<br>Data Club telah ditambahkan";	
						}
					else
						{ 
							$pesan = $pesan."<br>Gagal menambahkan data Club". mysql_error();
						}
				}
			else
				{
...
$result = mysql_query("DELETE FROM club WHERE id_club = ".$frm_no_ID_club);
    ...
    <script language="javascript">//confirmRefresh();
	    alert ('Data kota <? echo $frm_nama_club;?> telah di Hapus.');
...
//echo "<br>frm_nama_club=".$_POST['frm_nama_club'];
if ($frm_nama_club!='')  {
$result = mysql_query("SELECT *
						 FROM club 
						WHERE nama='".$_POST['frm_nama_club']."'");
...
<div id="stylized" class="myform"> 
  <form id="form" name="form"> 
    <h1>Cari Data Club</h1> 
    <p>Gunakan form dibawah ini untuk mencari Data Club</p> 
    <label>Nama <span class="small">ketik nama club</span> </label> 
    ...
    <input type="hidden" name="menu" value="club"> 
...
// PROSES UNTUK SEARCH (MODE=2)
	...
	if ($frm_s_nama!="")
	{
		 $sql .= " and (club.nama LIKE '%".$frm_s_nama."%')";
	}
...
$vlink="index.php";
$abc="?mode=2&frm_s_nama=$frm_s_nama&frm_s_jum_data=$frm_s_jum_data&menu=club";
...
if (mysql_num_rows($result)==0) {
    echo "<br><font color=FF0000><b>Data Club Belum Tersedia !</b></font><br>Mohon periksa kembali data yang Anda cari";
	echo "<br><br><a href=index.php?menu=club class=menu_left>:: Kembali</a>";
...
?> 
    <tr> 
      ...
      <td nowrap>
		  <a class='example7' href="edit_club.php?id_club=<?=$row["id_club"];?>&nama_club=<?=$row["nama"];?>"><?=$row["nama"];?></a>
	  </td> 
      <td nowrap><? echo $row["alamat"]; ?></td>
      <td nowrap><? //echo $row["kode_dosen1"]." - ".$row["nama_dosen1"] ; 
			$sql2="SELECT * FROM kota WHERE id_kota='".$row["kota_club"]."'";
			...
			?> </td> 
      <td nowrap> <? 
			$sql2="SELECT * FROM propinsi WHERE id_propinsi='".$row["propinsi_club"]."'";
			$result2 = mysql_db_query($DB,$sql2);
			if ($row2 = mysql_fetch_array($result2))
			{
				echo $row2["nama"];			
...
{
?> 
<FORM METHOD="Post" name="fexcel" id="fexcel" ACTION="club_export.php"> 
  ...
  <?
	...
	?> 
  <div align="right"> 
    <input name="excel"   type="image" onClick="document.fexcel.action='club_export.php?t=excel'" src="../image/Mexcel.jpg" align="middle" width="18" height="18"> 
&nbsp; | &nbsp; 
    <input name="printer" type="image" onClick="document.fexcel.action='club_export.php?t=printer'" src="../image/print.gif" align="middle" width="18" height="18"> 
  </div> 
</form> 
<?
}
...
{
	$result = mysql_query("INSERT INTO apr_kabupaten (id_member, tipe_member, status, admin_user)VALUES(".$_GET['id_club'].", 'club', 1, '".$_GET['admin_user']."')" );
	if ($result) 
		{
			$pesan = $pesan."<br>Data Club '".$_GET['frm_nama_club_club']."' telah di SETUJUI";	
		}
	else
		{ 
			$pesan = $pesan."<br>Gagal menambahkan data club '".$_GET['frm_nama_club_club']."' - mohon hubungi admin". mysql_error();
		}

}
if ($_GET['var_apr_kab']=="0")
{
	$result = mysql_query("INSERT INTO apr_kabupaten (id_member, tipe_member, status, admin_user)VALUES(".$_GET['id_club'].", 'club', 0, '".$_GET['admin_user']."')" );
	if ($result) 
		{
			$pesan = $pesan."<br>Data Club '".$_GET['frm_nama_club_club']."' telah di SET ULANG";	
		}
	else
		{ 
			$pesan = $pesan."<br>Gagal menambahkan data club '".$_GET['frm_nama_club_club']."' - mohon hubungi admin". mysql_error();
...
{
	$result = mysql_query("INSERT INTO apr_propinsi (id_member, tipe_member, status, admin_user)VALUES(".$_GET['id_club'].", 'club', 1, '".$_GET['admin_user']."')" );
	if ($result) 
		{
			$pesan = $pesan."<br>Data Club '".$_GET['frm_nama_club_club']."' telah di SETUJUI";	
		}
...
{
	$result = mysql_query("INSERT INTO apr_propinsi (id_member, tipe_member, status, admin_user)VALUES(".$_GET['id_club'].", 'club', 0, '".$_GET['admin_user']."')" );
	if ($result) 
		{
			$pesan = $pesan."<br>Data Club '".$_GET['frm_nama_club_club']."' telah di SET ULANG";	
		}
...
{
	$result = mysql_query("INSERT INTO apr_pusat (id_member, tipe_member, status, admin_user)VALUES(".$_GET['id_club'].", 'club', 1, '".$_GET['admin_user']."')" );
	if ($result) 
		{
			$pesan = $pesan."<br>Data Club '".$_GET['frm_nama_club_club']."' telah di SETUJUI";	
		}
...
{
	$result = mysql_query("INSERT INTO apr_pusat (id_member, tipe_member, status, admin_user)VALUES(".$_GET['id_club'].", 'club', 0, '".$_GET['admin_user']."')" );
	if ($result) 
		{
			$pesan = $pesan."<br>Data Club '".$_GET['frm_nama_club_club']."' telah di SET ULANG";	
		}
