...
/* 
   ...
   KEGUNAAN: MENAMPILKAN LAPORAN PENGABDIAN MASYARAKAT
*/
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="89%"><font color="#006699"><font color="#0099CC" size="1"><strong>LAPORAN ~</strong> 
      PENGABDIAN MASYARAKAT</font></font> </td>
...
$sql="SELECT pengabdian.id,
			 ...
			 pengabdian.jabatan,
			 pengabdian.judul,
			 DATE_FORMAT(pengabdian.mulai,\"%d/%m/%Y\") as tanggal_mulai,
			 DATE_FORMAT(pengabdian.selesai,\"%d/%m/%Y\") as tanggal_selesai,
			 pengabdian.tempat,
			 ...
			 pengabdian.jurusan,
			 pengabdian.man_kel
	    FROM pengabdian, sumber_dana, jurusan, dosen
	   WHERE pengabdian.kode_dosen=dosen.kode AND
	         pengabdian.id_sumber_dana=sumber_dana.id AND
			 pengabdian.jurusan=jurusan.id ";	
		//dosen.kode=pengabdian.kode_dosen and 
...
// PROSES UNTUK SEARCH (MODE=2)
	...
	{  
		if($frm_s_tanggal_mulai1!="" && $frm_s_tanggal_mulai2!="")
		{ $sql=$sql." and pengabdian.mulai between '".datetomysql($frm_s_tanggal_mulai1)."' and '".datetomysql($frm_s_tanggal_mulai2)."'"; }
		else
		{
			if($frm_s_tanggal_mulai1!="")
			{ $sql=$sql." and pengabdian.mulai >='".datetomysql($frm_s_tanggal_mulai1)."'"; }
			if($frm_s_tanggal_mulai2!="")
			{ $sql=$sql." and pengabdian.mulai <='".datetomysql($frm_s_tanggal_mulai2)."'"; }
		}
	}	
	if ($frm_s_tanggal_selesai1!="" || $frm_s_tanggal_selesai2!="")
	{  
		if($frm_s_tanggal_selesai1!="" && $frm_s_tanggal_selesai2!="")
		{ $sql=$sql." and pengabdian.selesai between '".datetomysql($frm_s_tanggal_selesai1)."' and '".datetomysql($frm_s_tanggal_selesai2)."'"; }
		else
		{
			if($frm_s_tanggal_selesai1!="")
			{ $sql=$sql." and pengabdian.selesai >='".datetomysql($frm_s_tanggal_selesai1)."'"; }
			if($frm_s_tanggal_selesai2!="")
			{ $sql=$sql." and pengabdian.selesai <='".datetomysql($frm_s_tanggal_selesai2)."'"; }
		}
	}
	if ($frm_s_jurusan != "all")
	{ $sql.=" and pengabdian.jurusan=".$frm_s_jurusan;}	
	if ($frm_s_kode!="")
	...
	//{ $sql=$sql." and pengabdian.id_jenis_kerjasama=".$frm_s_publikasi; }
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr> 
    <td width="89%"><font color="#006699"><font color="#0099CC" size="1"><strong>LAPORAN 
      ~</strong> PENGABDIAN MASYARAKAT</font></font></td>
    <td width="11%" nowrap bgcolor="#0099CC"><div align="center"><strong><font color="#FFFFFF">DOSEN</font></strong></div></td>
  </tr>
</table>
<hr size="1" color="#FF9900">
...
if (mysql_num_rows($result)==0) {
	...
	echo "<br><br><a href=lap_pengabdian.php class=menu_left>:: Kembali</a>";
...
{
?>
<FORM METHOD="Post" name="fexcel" id="fexcel" ACTION="lap_pengabdian_export.php">
...
?>
		<tr>
			...
			<td nowrap align="right" valign="top">
				Edit <input name="edit" onClick="document.fexcel.action='pengabdian_entry.php?frm_id_abdi=<?php echo $row["id"];?>&frm_kode=<?php echo $row["kode"];?>'"   type="image" src="../img/edit.png" width="16" height="16"><br>
				Hapus <input name="hapus"  onclick="if(confirm('Hapus ?')){this.form.action='pengabdian_entry.php?act=2&frm_id=<?php echo $row["id"];?>';this.form.submit()} else{return false};" type="image" src="../img/hapus.png" width="11" height="13"> 
			</td>
			<? }?>
			...
			<!--td nowrap valign="top"-->
			   <?
					/*$sql_publikasinya="SELECT nama
									     FROM jenis_kerjasama
									    WHERE id=".$row["id_jenis_kerjasama"];
					$result_publikasinya = mysql_query($sql_publikasinya);
					$row_publikasinya = mysql_fetch_array($result_publikasinya);
					echo $row_publikasinya["nama"];
					echo $row["id_jenis_kerjasama"];
					*/
			   ?>
			<!--/td-->
...
?><tr>
		<td colspan=15>
			<input name="excel" type="image" onClick="document.fexcel.action='lap_pengabdian_export.php?t=excel'" src="../img/Mexcel.jpg" align="bottom" width="18" height="18"> 
			Export ke File Excel
			<input name="printer" type="image"  onClick="document.fexcel.action='lap_pengabdian_export.php?t=printer'" src="../img/print.gif" align="bottom" width="18" height="18"> 
			Print
		</td>
	</tr>
</table>
