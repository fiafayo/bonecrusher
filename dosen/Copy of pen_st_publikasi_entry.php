...
// Kode dan nama harus diisi 
	...
	if (($frm_no_st_pub_now=='') or ($frm_jenis=='') or ($frm_kode_dsn1=='') or ($frm_ska_publikasi=='') or ($frm_tipe_publikasi=='') or ($frm_judul=='') or ($frm_ISBN=='') or 
	($frm_id_sumber_dana=='') or ($frm_dana=='') or  ($frm_TET=='') or ($frm_tugas=='') or ($frm_status=='') or ($frm_hari_go=='') or 
	($frm_tgl_go=='') or ($frm_pukul_go=='') or ($frm_tempat_go=='') or ($frm_transportasi_go=='') or ($frm_hari_dtg=='') or 
	($frm_tgl_dtg=='') or ($frm_pukul_dtg=='') or ($frm_transportasi_dtg==''))
		{
			$error = 1;
...
// BEGIN UBAH TABEL #########################################################################################################					
					...
					{
							$result_1_2 = mysql_query("INSERT INTO publikasi ( `no_legalitas` , `no_st_pub` , `urut_st_pub` , `jenis` , `kode_kary` , `kode_kary2`, 
																   `kode_kary3`, `kode_kary4`, `kode_kary5`, `status` , `TET_sub` , `TNET_sub` , 
																   `tugas` , `hari_go` , `tgl_go` , `pukul_go` , `tempat_go` , `transport_go` , 
																   `hari_dtg` , `tgl_dtg` , `pukul_dtg` , `transport_dtg` , `biaya` , 
																   `L_ap_tax` , `L_fiskal` , `L_visa` , `L_saku` , `L_akomo` , `L_other` , `ndt_terakhir`) 
														  VALUES ( '".$frm_no_legalitas_now."', '".$frm_no_st_pub_now."', NULL, ".$frm_jenis.", '".$frm_kode_dsn1."', 
														           '".$frm_kode_dsn2."', '".$frm_kode_dsn3."', '".$frm_kode_dsn4."', 
																   '".$frm_kode_dsn5."', '".$frm_status."', '".$frm_TET."', '".$frm_TNET."', 
																   '".$frm_tugas."', '".$frm_hari_go."','".$frm_tgl_go."', '".$frm_pukul_go."', 
																   '".$frm_tempat_go."', '".$frm_transportasi_go."', '".$frm_hari_dtg."', 
																   '".$frm_tgl_dtg."', '".$frm_pukul_dtg."','".$frm_transportasi_dtg."',
																   '".$frm_biaya."',".$chk_airport.",".$chk_fiskal.",".$chk_visa.",
																   ".$chk_uang_saku.",".$chk_akomodasi.",'".$frm_lainnya."','".$frm_ndt."') " );
					        if ($result_1_2) 
							{
...
$result = mysql_query("Select publikasi.no_legalitas,
							  ...
							  tulisan_ilmiah.dana_asing
					     FROM publikasi
					LEFT JOIN tulisan_ilmiah 
					       ON publikasi.no_st_pub = tulisan_ilmiah.no_st_pub
					    WHERE publikasi.no_st_pub='".$frm_no_st_pub_now."'");
						...
						if ($row = mysql_fetch_array($result)) {
							...
							if ($frm_tgl_publikasi=="00/00/0000") {
...
<html>
<head>
<meta http-equiv="Refresh" content="300; URL=pen_st_publikasi_entry.php">
  ...
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" class="body">
    ...
    <tr>
      ...
      <td><input name="frm_tgl_publikasi" type="text" class="tekboxku" id="frm_tgl_publikasi" value="<?php echo $frm_tgl_publikasi; ?>" size="10" maxlength="10">
        <A Href="javascript:show_calendar('publikasi.frm_tgl_publikasi',0,0,'DD/MM/YYYY')"> <img src="../img/date.png" width="22" height="22" align="absbottom" border=0> </A>(dd/mm/yyyy)</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
    ...
    <tr>
      ...
      <td><strong>:</strong></td>
      <td>Rp.
      <input type="text" name="frm_dana" id="frm_dana" onkeypress="return isNumberKey(event)"  value="<?php echo $frm_dana; ?>" class="tekboxku">
      <font color="#FF0000">*</font></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      ...
      <td><strong>:</strong></td>
      <td><input type="text" name="frm_dana_asing" id="frm_dana_asing" onkeypress="return isNumberKey(event)"  value="<?php echo $frm_dana_asing; ?>" class="tekboxku"></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td nowrap>&nbsp;</td>
    ...
    <tr>
      ...
      <td>
		   <select name="frm_kode_dsn1" id="frm_kode_dsn1" class="tekboxku">
					<option <?php if ($frm_kode_dsn1==''){echo "selected";}?> value="">-- Kode Dosen --</option>
					<?php
    ...
    <tr>
      ...
      <td><select name="frm_kode_dsn2" id="frm_kode_dsn2" class="tekboxku">
        <option <?php if ($frm_kode_dsn2==''){echo "selected";}?> value="">-- Kode Dosen --</option>
        <?php
					$result1 = @mysql_query("SELECT kode, nama FROM dosen WHERE kode LIKE '6%' order by kode ASC");
					$c=0;
					while ($row1=@mysql_fetch_object($result1))  {
    ...
    <tr>
	  ...
	  <select name="frm_kode_dsn3" id="frm_kode_dsn3" class="tekboxku">
        <option <?php if ($frm_kode_dsn3==''){echo "selected";}?> value="">-- Kode Dosen --</option>
        <?php
					$result1 = @mysql_query("Select kode, nama from dosen WHERE kode LIKE '6%' order by kode ASC");
					$c=0;
					while ($row1=@mysql_fetch_object($result1))  {
    ...
    <tr>
      ...
      <td><select name="frm_kode_dsn4" id="frm_kode_dsn4" class="tekboxku">
        <option <?php if ($frm_kode_dsn4==''){echo "selected";}?> value="">-- Kode Dosen --</option>
        <?php
					$result1 = @mysql_query("Select kode, nama from dosen WHERE kode LIKE '6%' order by kode ASC");
					$c=0;
					while ($row1=@mysql_fetch_object($result1))  {
    ...
    <tr>
      ...
      <td>
	   <select name="frm_kode_dsn5" id="frm_kode_dsn5" class="tekboxku">
        <option <?php if ($frm_kode_dsn5==''){echo "selected";}?> value="">-- Kode Dosen --</option>
        <?php
					$result1 = @mysql_query("Select kode, nama from dosen WHERE kode LIKE '6%' order by kode ASC");
					$c=0;
					while ($row1=@mysql_fetch_object($result1))  {
    ...
    <tr>
      ...
      <td colspan="3"><table width="100%"  border="0" cellspacing="0" cellpadding="2">
        ...
        <tr>
          ...
          <td valign="top"><strong>:</strong></td>
          <td><input name="frm_biaya" id="frm_biaya" type="text" onKeyPress="return isNumberKey(event)"  class="tekboxku" size="20" maxlength="30" value="<?php echo $frm_biaya; ?>" >            <font color="#FF0000">*</font></td>
        </tr>
        <tr>
