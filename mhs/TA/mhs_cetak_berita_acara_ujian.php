...
/* 
   ...
   KEGUNAAN     : buat CETAK BERITA ACARA UJIAN TA
	...
	if ($error !=1) // Jika semua isian form valid 
		{
					...
					if ($row = mysql_fetch_array($result)) 
					{
						...
						<script language="JavaScript">
							...
							//{
								newwindow=window.open('form_cetak_berita_acara.php?nrp=<? echo $frm_nrp;?>','name','top=0,left=510,scrollbars=yes');
...
if ($frm_nrp!='')  {
		$result = mysql_query("SELECT * FROM `master_mhs` WHERE `master_mhs`.`NRP` =  '".$frm_nrp."'");
		if ($row = mysql_fetch_array($result)) {
			$frm_exist=1;
			$frm_nama = $row["NAMA"];
			$frm_no_ST_TA_terakhir = $row2["N_ST"];//}
		}else
		{$frm_exist=0; header("Location: mhs_cetak_berita_acara_ujian.php"); }
}
}
...
<body class="body">
<form name="mhs_cetak_berita_acara" id="mhs_cetak_berita_acara" action="mhs_cetak_berita_acara_ujian.php" method="post">
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" class="body">
      ...
      <td colspan="4"> <hr size="1" color="#FF9900"> <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr> 
            <td width="89%"><font color="#006699"><font color="#0099CC" size="1"><strong>CETAK 
              ~</strong> BERITA ACARA UJIAN TA</font></font></td>
    ...
    <tr> 
      ...
      <td><input name="frm_nrp" type="text" class="tekboxku" id="frm_nrp" onBlur="document.mhs_cetak_berita_acara.submit()" value="<?php echo $frm_nrp; ?>" size="10" maxlength="10" > 
