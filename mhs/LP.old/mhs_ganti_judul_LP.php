		...
		// data id tidak ada, berarti record baru
			if ($frm_exist!=1) 
			...
			else
				{
					...
					$result = mysql_query("UPDATE ganti_judul_lp set `judul_lama`='$frm_judul_semula', `judul_baru`='$frm_judul_baru', `tgl_aju` ='$frm_tgl_aju' , `tgl_ganti` ='$frm_tgl_ganti' where `nrp`=$frm_nrp");
					if ($result) 
						{
...
$result = mysql_query("delete from ganti_judul_LP where nrp = ".$frm_nrp);
...
if ($isDebug) {
    ...
    if ($flog) {
        ...
        fclose($flog);
							...
							{$frm_exist=0; header("Location: mhs_ganti_judul_LP.php"); }
