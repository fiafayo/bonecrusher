...
// No.SK harus diisi
	...
	if ($error !=1) // Jika isian form valid 
		{
					$sql_cetak_lampiran_SK_Pen="SELECT daftar_uji_lp.NO_SK
								     			FROM daftar_uji_lp 
								     			WHERE daftar_uji_lp.NO_SK='".$frm_no_SK."'";
					$result = mysql_query($sql_cetak_lampiran_SK_Pen);
					if ($row = mysql_fetch_array($result)) 
					{
						...
						<script language="JavaScript">
							...
							//{
								newwindow=window.open('form_cetak_SK_LP.php?no_sk=<? echo $frm_no_SK;?>&periode=<? echo $frm_periode;?>&thn_ajar=<? echo $frm_id_tahun_ajar;?>','name','top=0,left=200,scrollbars=yes');
...
<body class="body">
<form name="mhs" id="mhs" action="mhs_cetak_SK_LP.php" method="post">
      ...
      <td colspan="4"> <hr size="1" color="#FF9900"> <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr> 
            <td width="89%"><font color="#006699"><font color="#0099CC" size="1"><strong>CETAK 
              ~</strong> MAHASISWA SK PENELITIAN </font></font></td>
