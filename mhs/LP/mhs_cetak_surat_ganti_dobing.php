...
// Form harus diisi
	...
	if ($error!=1) // Jika semua isian form valid 
		{
					$sql_cek_mhs="SELECT `no_surat_lp`.`NRP`
								  FROM no_surat_lp 
					...
					{
						//$pesan=$pesan."<br>ADA";
						$result_cek = mysql_query("SELECT `NRP`, `N_GANDOS`, `N_URUT_GANDOS` FROM no_surat_lp WHERE `no_surat_lp`.`NRP`='".$frm_nrp."'");
						...
						if ($row_result_cek >= 1)
						{
							...
							{
								$result_update1 = mysql_query("UPDATE no_surat_lp set `N_GANDOS`='$frm_no_surat_ganti_dobing_baru' WHERE `NRP`='$frm_nrp'");
							}
							else
							{
								$result_update1 = mysql_query("UPDATE no_surat_lp set `N_GANDOS`='$frm_no_surat_ganti_dobing_baru', `N_URUT_GANDOS`='$frm_no_urut_surat_ganti_dobing_terakhir' WHERE `NRP`='$frm_nrp'");
							}
							
							$result_update2 = mysql_query("UPDATE ganti_dobing_lp set `NO_GANTI`='$frm_no_surat_ganti_dobing_baru' WHERE `NRP`='$frm_nrp'");
							if ($result_update1)
							{
									?>
									<script language="JavaScript">
											newwindow=window.open('form_cetak_ganti_dobing.php?nrp=<? echo $frm_nrp;?>&tgl_surat=<? echo $frm_tgl_surat_dibuat;?>','name','top=0,left=510,scrollbars=yes');
						...
						{
							$error = 1;
							$pesan = $pesan."<br>Maaf, Mahasiswa belum pernah mengajukan LP. Gagal Mencetak 1.". mysql_error();
						}
					}
					else
					{
						$pesan=$pesan."<br>Maaf, Mahasiswa belum pernah mengajukan LP. Gagal Mencetak 2.";
					}			
		}
	}
...
$result = mysql_query("SELECT  master_mhs.NRP,
						...
						FROM  `master_mhs` LEFT JOIN `no_surat_lp` ON `master_mhs`.`NRP` = `no_surat_lp`.`NRP`
						WHERE `master_mhs`.`NRP` =  '".$frm_nrp."'");
							...
							if ($row = mysql_fetch_array($result)) {
								...
								$result2 = mysql_query("SELECT N_GANDOS, N_URUT_GANDOS FROM no_surat_lp WHERE N_GANDOS<>'' ORDER BY N_URUT_GANDOS DESC LIMIT 1");
							...
							{
                                                            $frm_exist=0;                                                         
                                                            //header("Location: mhs_cetak_surat_ganti_dobing.php");                                                         
                                                        }		
}
}
