...
$frm_kode_dosen_lama2= ( isset( $_REQUEST['frm_kode_dosen_lama2'] ) ) ? $_REQUEST['frm_kode_dosen_lama2'] : null;
                            ...
                            if ($result) {
                                ...
                                if ($row) {
                                    $nrpMhs2 = $row['NRP'];
                                        ...
                                        if ($frm_exist!=1) 
                                                {
                                                        $sqlInsert="INSERT INTO ganti_dobing_lp( `NRP` ,`kode_dobing_lama` ,  `kode_dobing_baru` , `tgl_ganti` ,  `tgl_aju` , `disetujui` , NO_GANTI) VALUES ( '".$frm_nrp."', '".$frm_kode_dosen_lama."', '".$frm_kode_dosen_baru."', '".$frm_tgl_ganti."', '".$frm_tgl_aju."', '".$frm_setuju."','$stLp') " ;
                                                        $result = mysql_query($sqlInsert);
                                                        if ($nrpMhs2) {
                                                            $sqlInsert="INSERT INTO ganti_dobing_lp( `NRP` ,`kode_dobing_lama` ,  `kode_dobing_baru` , `tgl_ganti` ,  `tgl_aju` , `disetujui`, NO_GANTI ) VALUES ( '".$nrpMhs2."', '".$frm_kode_dosen_lama."', '".$frm_kode_dosen_baru."', '".$frm_tgl_ganti."', '".$frm_tgl_aju."', '".$frm_setuju."','$stLp') " ;
                                                            $result = mysql_query($sqlInsert);
                                                        }
                                                        ...
                                                        if ($result) 
                                                                ...
                                                                { 
                                                                        $pesan = $pesan."<br>Gagal menambahkan data $sqlInsert";
                                                                }
                                                }
                                        else
                                                {
                                                        $result = mysql_query("UPDATE ganti_dobing_lp set `kode_dobing_lama`='$frm_kode_dosen_lama', `kode_dobing_baru`='$frm_kode_dosen_baru', `tgl_ganti` ='$frm_tgl_ganti' , `tgl_aju` ='$frm_tgl_aju' , `disetujui` ='$frm_setuju' where (NRP='$frm_nrp') or (NRP='$nrpMhs2')  ");
                                                        if ($result) 
                                                                {
...
{ // masuk selain simpan/hapus/batal  atau ada errornya
...
if ($result) {
    ...
    $row=false;
							...
							if ($row) { //data sudah pernah ada
								...
								if (!$frm_kode_dosen_lama)
									{
										$sql_dobing = "SELECT KODOS1,KODOS2 from master_lp where NRP='".$frm_nrp."'";
                                                                                //echo $sql_dobing;
										$result_dobing = mysql_query($sql_dobing);
										if ($row_dobing = mysql_fetch_array($result_dobing))
										{
											$frm_kode_dosen_lama=$row_dobing["KODOS1"];
                                                                                        $frm_kode_dosen_lama2=$row_dobing["KODOS2"];
										}
									}                                                            
							...
							} else
							{
                                                            ...
                                                            $frm_exist=0; 
                                                            //header("Location: mhs_ganti_pembimbing_LP.php"); 
  ...
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" class="body">
    ...
    <tr>
      ...
      <td><select name="frm_kode_dosen_lama" id="frm_kode_dosen_lama" class="tekboxku">
          ...
          <?php 
				$sqlDosen="select kode, nama from dosen   ORDER BY kode";
				
				$result = @mysql_query($sqlDosen);
				while ($row=@mysql_fetch_object($result))  {
                                    if ( ($frm_kode_dosen_lama==$row->kode) || ($frm_kode_dosen_lama2==$row->kode)  ) {
