...
/* 
   ...
   KEGUNAAN     : CETAK NILAI DOSEN PENGUJI 2 LP
...
$frm_no_ST= ( isset( $_REQUEST['frm_no_ST'] ) ) ? $_REQUEST['frm_no_ST'] : null;
...
// No.ST harus diisi
	...
	if ($error !=1) // Jika isian form valid 
		{
					$sql_cetak_nilai_doji2="SELECT no_surat_lp.N_ST
								    		FROM no_surat_lp 
								    		WHERE no_surat_lp.N_ST='".$frm_no_ST."'";
					$result = mysql_query($sql_cetak_nilai_doji2);
					...
					if ($row) 
					{
						...
						<script language="JavaScript">
							...
							//{
								newwindow=window.open('form_cetak_nilai_doji_LP2.php?no_st=<? echo $frm_no_ST;?>&periode=<? echo $frm_periode;?>&thn_ajar=<? echo $frm_id_tahun_ajar;?>','name','top=0,left=200,scrollbars=yes');
...
<body class="body">
<form name="mhs" id="mhs" action="mhs_cetak_nilai_doji_LP2.php" method="post">
      ...
      <td colspan="4"> <hr size="1" color="#FF9900"> <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr> 
            <td width="89%"><font color="#006699"><font color="#0099CC" size="1"><strong>CETAK 
              ~</strong> NILAI DOSEN PENGUJI 2 </font></font></td>
